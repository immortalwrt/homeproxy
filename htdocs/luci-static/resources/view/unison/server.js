...
function getServiceStatus() {
	return L.resolveDefault(callServiceList('unison'), {}).then((res) => {
		let isRunning = false;
		try {
			isRunning = res['unison']['instances']['sing-box-s']['running'];
		} catch (e) { }
		return isRunning;
	});
}
...
function renderStatus(isRunning, version) {
	...
	let renderHTML;
	if (isRunning)
		renderHTML = spanTemp.format('green', _('Unison Server'), version, _('RUNNING'));
	else
		renderHTML = spanTemp.format('red', _('Unison Server'), version, _('NOT RUNNING'));
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('unison', _('Unison Server'),
		...
		s = m.section(form.NamedSection, 'server', 'unison', _('Global settings'));
		...
		*/
		...
		o.onchange = function(ev, section_id, value) {
			...
			let tls_element = this.map.findElement('id', 'cbid.unison.%s.tls'.format(section_id)).firstElementChild;
		...
		o.value('/etc/unison/certs/server_publickey.pem');
		...
		o.depends({'tls': '1', 'tls_cert_path': '/etc/unison/certs/server_publickey.pem'});
		...
		o.value('/etc/unison/certs/server_privatekey.pem');
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': null});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': null});
		...
		o.inputstyle = 'action';
		o.inputtitle = _('Upload...');
		o.depends({'tls': '1', 'tls_key_path': '/etc/unison/certs/server_privatekey.pem'});
