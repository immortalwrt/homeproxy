...
po2lmo "$PKG_DIR/po/zh_Hans/unison.po" "$TEMP_PKG_DIR/usr/lib/lua/luci/i18n/unison.zh-cn.lmo"

if [ "$PKG_MGR" == "apk" ]; then
	find "$TEMP_PKG_DIR" -type f,l -printf '/%P\n' | sort > "$TEMP_PKG_DIR/lib/apk/packages/$PKG_NAME.list"
	echo "/etc/config/unison" >> "$TEMP_PKG_DIR/lib/apk/packages/$PKG_NAME.conffiles"
...
default_prerm' > "$TEMP_DIR/pre-deinstall"
	...
	apk mkpkg \
		...
		--info "arch:all" \
		--info "origin:https://github.com/immortalwrt/unison" \
		--info "url:" \
	...
	cat > "$TEMP_PKG_DIR/CONTROL/control" <<-EOF
		...
		Source: https://github.com/immortalwrt/unison
		SourceName: $PKG_NAME
		Section: luci
	...
	echo -e "/etc/config/unison" > "$TEMP_PKG_DIR/CONTROL/conffiles"
